...
exports.renderServicesPage = async (req, res) => {
    try {
        ...
        const services = result.records.map(s => {
            ...
            let margem = 0;
            if (rev > 0) margem = ((rev - cost) / rev) * 100;
            else if (cost > 0) margem = -100;
...
exports.getServiceDetails = async (req, res) => {
    ...
    try {
        ...
        let rFin = { records: [] };
        try { rFin = await conn.query(`SELECT Id, Valor__c, DataVencimento__c, Competencia__c, Status__c, Descricao__c FROM ParcelaFinanceira__c WHERE Servico__c = '${id}'`); } catch(err) { console.error("Erro busca parcelas:", err.message); }

        if (rSvc.totalSize === 0) return res.status(404).json({ message: "Not found" });
        const svc = rSvc.records[0];
        let margemReal = svc.ReceitaRealizada__c > 0 ? ((svc.ReceitaRealizada__c - (svc.CustoRealizado__c || 0)) / svc.ReceitaRealizada__c) * 100 : 0;
...
exports.getServiceRealizedData = async (req, res) => {
    ...
    try {
        const conn = await getSfConnection();
        const orcResults = await conn.query(`SELECT Id, Alocacao__c, AlocacaoPrevista__c, Competencia__c, ReceitaPrevista__c, CustoPrevisto__c, ReceitaRealizada__c, CustoRealizado__c, HorasRealizadas__c FROM OrcamentoCompetencia__c WHERE Servico__c = '${id}' ORDER BY Competencia__c ASC`);
        const consolidated = {};
        orcResults.records.forEach(orc => {
            ...
            if (orc.AlocacaoPrevista__c) { data.baseline.revenue += orc.ReceitaPrevista__c || 0; data.baseline.cost += orc.CustoPrevisto__c || 0; }
            if (orc.Alocacao__c) { data.forecast.revenue += orc.ReceitaPrevista__c || 0; data.forecast.cost += orc.CustoPrevisto__c || 0; data.realized.revenue += orc.ReceitaRealizada__c || 0; data.realized.cost += orc.CustoRealizado__c || 0; data.realized.hours += orc.HorasRealizadas__c || 0; }
        });
        const timeLogs = await conn.query(`SELECT Responsavel__r.Name, SUM(Horas__c) hrs, SUM(ValorTotalLancamento__c) cost, CALENDAR_MONTH(DiaPeriodo__r.Data__c) m, CALENDAR_YEAR(DiaPeriodo__r.Data__c) y FROM LancamentoHora__c WHERE Servico__c = '${id}' AND Status__c = 'Faturado' GROUP BY Responsavel__r.Name, CALENDAR_MONTH(DiaPeriodo__r.Data__c), CALENDAR_YEAR(DiaPeriodo__r.Data__c)`);
        const revenueLogs = await conn.query(`SELECT OrcamentoCompetencia__r.Alocacao__r.Pessoa__r.Name person, SUM(ValorDistribuido__c) rev, CALENDAR_MONTH(OrcamentoCompetencia__r.Competencia__c) m, CALENDAR_YEAR(OrcamentoCompetencia__r.Competencia__c) y FROM DistribuicaoReceita__c WHERE OrcamentoCompetencia__r.Servico__c = '${id}' GROUP BY OrcamentoCompetencia__r.Alocacao__r.Pessoa__r.Name, CALENDAR_MONTH(OrcamentoCompetencia__r.Competencia__c), CALENDAR_YEAR(OrcamentoCompetencia__r.Competencia__c)`);
        const revMap = {};
        revenueLogs.records.forEach(r => { revMap[`${r.person}_${r.y}-${r.m}`] = (revMap[`${r.person}_${r.y}-${r.m}`] || 0) + r.rev; });
        res.json({ success: true, consolidated: Object.values(consolidated).sort((a,b) => a.month.localeCompare(b.month)), timeLogs: timeLogs.records.map(r => ({ person: r.Name, hours: r.hrs, cost: r.cost, revenue: revMap[`${r.Name}_${r.y}-${r.m}`] || 0, period: `${r.y}-${String(r.m).padStart(2,'0')}-01` })) });
...
exports.saveService = async (req, res) => {
    ...
    try {
        ...
        if (result.success) {
            ...
            if (data.commercial) {
                for (const item of data.commercial) {
                    ...
                    if (item.monthlyData && item.monthlyData.length > 0) {
                        ...
                        await conn.sobject('OrcamentoCompetencia__c').create(item.monthlyData.map(m => ({ AlocacaoPrevista__c: commIdMap[item.id], Servico__c: serviceId, Competencia__c: m.date, ReceitaPrevista__c: m.rev, CustoPrevisto__c: m.cost })));
                    }
                }
            }
            if (data.execution) {
                for (const item of data.execution) {
                    ...
                    if (item.monthlyData && item.monthlyData.length > 0) {
                        ...
                        if (exOrcs.totalSize > 0) await conn.sobject('OrcamentoCompetencia__c').destroy(exOrcs.records.map(r => r.Id));
                        await conn.sobject('OrcamentoCompetencia__c').create(item.monthlyData.map(m => ({ Alocacao__c: ret.id || item.id, Servico__c: serviceId, Competencia__c: m.date, ReceitaPrevista__c: m.rev, CustoPrevisto__c: m.cost })));
                    }
                }
            }
            ...
            await conn.sobject('Servico__c').update({ Id: serviceId, ReceitaVendida__c: tRev, CustoVendido__c: tCost, MargemVendida__c: tRev > 0 ? ((tRev - tCost) / tRev) * 100 : 0, ReceitaPrevista__c: allocTotals.records[0].rev || 0, CustoPrevisto__c: allocTotals.records[0].cost || 0, MargemPrevista__c: (allocTotals.records[0].rev || 0) > 0 ? (((allocTotals.records[0].rev || 0) - (allocTotals.records[0].cost || 0)) / (allocTotals.records[0].rev || 0)) * 100 : 0 });
