...
async function run() {
    ...
    console.log(`🚀 Iniciando configuração para Ribamar (${targetId})...`);
    ...
    // --- 2. CRIAR NOVAS ---
    const searchTerms = ['Carapreta', 'Coty', 'Audi', 'Cherry', 'Zanchetta', 'Compre e Alugue Agora', 'PSAFE', 'C3C Software | Suporte'];
...
📦 Encontrados ${resServ.totalSize} serviços para alocação.`);
    ...
    const newAllocs = resServ.records.map(s => {
        console.log(`   - Criando para: ${s.Name}`);
        return {
            Pessoa__c: targetId,
            Servico__c: s.Id,
