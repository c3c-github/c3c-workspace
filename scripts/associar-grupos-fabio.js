...
async function run() {
    ...
    const targetGroups = ['GESTAO_SUPORTE', 'GESTOR', 'OPERACAO'];
    ...
    targetGroups.forEach(code => {
        if (existingGroups.has(code)) {
            console.log(`ℹ️  Já possui acesso: ${code}`);
        } else if (groupMap[code]) {
            console.log(`➕ Preparando associação: ${code}`);
            toCreate.push({
                Pessoa__c: targetId,
    ...
    try {
        ...
        console.log(`
✅ ${success} associações criadas com sucesso.`);
        if (success < ret.length) console.error(ret.filter(r => !r.success));
    } catch (e) {
