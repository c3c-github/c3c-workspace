...
exports.getMyAllocations = async (req, res) => {
  try {
    ...
    const data = result.records.map((r) => ({
      ...
      accountName:
        r.Servico__r && r.Servico__r.Conta__r
          ? r.Servico__r.Conta__r.Name
          : "-",
...
exports.getPeriods = async (req, res) => {
  try {
    ...
    if (type === "user") {
      ...
      const [result, resDefaultRaw] = await Promise.all([
        conn.query(query),
        conn.query(qDefault)
      ]);

      let resDefault = resDefaultRaw;

      // FALLBACK PARA RH/ADMIN: Se não achou nada pro líder, pega o período aberto mais antigo da ORG
      if (!resDefault.records || resDefault.records.length === 0) {
        const qFallback = `SELECT DataInicio__c, DataFim__c FROM Periodo__c WHERE Status__c != 'Finalizado/Pago' ORDER BY DataInicio__c ASC LIMIT 1`;
        const resFallback = await conn.query(qFallback);
        if (resFallback.records && resFallback.records.length > 0) {
          resDefault = {
            records: [{
              DiaPeriodo__r: {
                Periodo__r: {
                  DataInicio__c: resFallback.records[0].DataInicio__c,
                  DataFim__c: resFallback.records[0].DataFim__c
                }
              }
            }]
          };
      ...
      const suggestedDefault = resDefault.records[0]
        ? `${resDefault.records[0].DiaPeriodo__r.Periodo__r.DataInicio__c}_${resDefault.records[0].DiaPeriodo__r.Periodo__r.DataFim__c}`
        : null;

      console.log('DEBUG CICLO SUGERIDO:', { suggestedDefault, emailLider });
...
exports.getProjects = async (req, res) => {
  try {
    ...
    const [resAlloc, resLanc] = await Promise.all([
      conn.query(qAlloc),
      conn.query(qLanc)
    ]);
    ...
    const getOrInitProject = (row) => {
      ...
      if (!projectsMap[sId]) {
        ...
        const plannedHours = isSupport
          ? sObj.Contrato__r?.HorasContratadas__c || 0
          : 0;
    ...
    resLanc.records.forEach((row) => {
      ...
      if (
        [
          "Em aprovação do serviço",
          "Rascunho",
          "Reprovado serviço",
          "Reprovado RH"
        ].includes(row.Status__c)
      ) {
        p.statusUI = "Aberto";
      }
    });
    ...
    const result = Object.values(projectsMap).map((p) => {
      ...
      let percent =
        p.metrics.alocado > 0
          ? Math.round((p.metrics.ponderado / p.metrics.alocado) * 100)
          : p.metrics.ponderado > 0
            ? 100
            : 0;
  ...
  try {
    ...
    resAlloc.records.forEach((row) => {
      ...
      resourcesMap[pId] = {
        ...
        alocado: diasUteis * info.carga * percent,
    ...
    resLanc.records.forEach((row) => {
      ...
      if (!resourcesMap[pId] && row.Pessoa__c) {
        ...
        resourcesMap[pId] = {
          id: row.Pessoa__c,
          name: row.Pessoa__r ? row.Pessoa__r.Name : "Colaborador",
          alocado: 0,
          periodStatus: info.status,
          totalRealizado: 0,
          horasNormais: 0,
      ...
      if (resourcesMap[pId]) {
        ...
        if (
          ["Em aprovação do RH", "Aprovado", "Faturado", "Fechado"].includes(st)
        )
          r.countApproved++;
        else if (["Reprovado serviço", "Reprovado RH"].includes(st))
          r.countRejected++;
      }
    });
    ...
    const result = Object.values(resourcesMap).map((r) => {
      let percent = 0;
      if (r.alocado > 0)
        percent = Math.round((r.totalRealizado / r.alocado) * 100);
...
exports.getResourceActivities = async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ error: "Identificadores de serviço ou pessoa inválidos." });
    }
...
exports.handleApprovalAction = async (req, res) => {
  const { serviceId, personId, action, inicio, fim, motivo, entryIds } =
    req.body;
  const novoStatus =
    action === "approve" ? "Em aprovação do RH" : "Reprovado serviço";
  ...
  try {
    ...
    if (resultBusca.totalSize === 0)
      return res.json({
        success: false,
        message: "Nenhum lançamento pendente encontrado."
      });
    ...
    if (periodIds.length > 0) {
      if (action === "reject") {
        ...
        const periodUpdates = periodIds.map((id) => ({
          Id: id,
          Status__c: "Aberto"
        }));
        ...
        for (const pId of periodIds) {
          ...
          if (totalPending === 0) {
            await conn
              .sobject("Periodo__c")
              .update({ Id: pId, Status__c: "Aguardando Aprovação RH" });
          }
        }
      }
    }
...
exports.getDashboardMetrics = async (req, res) => {
  try {
    ...
    if (scope === "personal") {
      ...
      const cargaDiaria =
        periodoRecord && periodoRecord.ContratoPessoa__r
      ...
      const nfRecord =
        periodoRecord &&
        periodoRecord.NotasFiscais__r &&
        periodoRecord.NotasFiscais__r.records
          ? periodoRecord.NotasFiscais__r.records[0]
          : null;
      ...
      if (periodoRecord) {
        ...
        resComp.records.forEach((r) => {
          ...
          diasUteis += count;
          if (r.DiaCompleto__c === true || r.DiaCompleto__c === "true")
            diasCompletos += count;
        });
      }
      ...
      resLanc.records.forEach((r) => {
        ...
        if (["Reprovado serviço", "Reprovado RH"].includes(r.Status__c))
          totalReprovadas += h;
      });
      ...
      const saldoBanco =
        resBanco.records[0] && resBanco.records[0].total
          ? resBanco.records[0].total
          : 0;

      let efic =
        totalAlocado > 0
          ? Math.round((totalLanc / totalAlocado) * 100)
          : totalLanc > 0
            ? 100
            : 0;
    ...
    const [resLanc, resAloc] = await Promise.all([
      conn.query(
        `SELECT Status__c, SUM(Horas__c) total FROM LancamentoHora__c WHERE Servico__r.Lider__r.Email__c = '${emailLider}' AND DiaPeriodo__r.Data__c >= ${inicio} AND DiaPeriodo__r.Data__c <= ${fim} AND ${FILTRO_HORAS} GROUP BY Status__c`
      ),
      conn.query(
        `SELECT SUM(HorasAlocadas__c) totalHorasDia FROM Alocacao__c WHERE Servico__r.Lider__r.Email__c = '${emailLider}' AND DataInicio__c <= ${fim} AND (DataFim__c >= ${inicio} OR DataFim__c = NULL)`
      )
    ]);

    let totalAlocado = 0;
    resAloc.records.forEach(
      (r) => (totalAlocado += (r.totalHorasDia || r.expr0 || 0) * 21)
    );
    let totalLanc = 0,
      totalPend = 0;
    resLanc.records.forEach((r) => {
      ...
      if (
        [
          ...
          "Reprovado serviço",
          "Reprovado RH"
        ].includes(r.Status__c)
      )
        totalPend += h;
    });
    let efic =
      totalAlocado > 0
        ? Math.round(((totalLanc - totalPend) / totalAlocado) * 100)
        : 0;
    res.json({
      totalAlocadas: totalAlocado,
      totalLancadas: totalLanc,
