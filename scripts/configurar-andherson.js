...
async function run() {
    ...
    console.log(`🚀 Iniciando configuração para Andherson Grangeiro (${targetId})...`);
    ...
    // --- 2. CRIAR NOVAS ---
    const searchTerms = ['Vindi', 'Abrasce', 'Aster', 'Célebre', 'Celebre', 'C3C Software | Suporte'];
...
📦 Encontrados ${resServ.totalSize} serviços para alocação.`);
    ...
    const newAllocs = resServ.records.map(s => {
        ...
        let pct = 0;
        if (name.includes('vindi')) pct = 100;
